<template>
  <v-container>
    <v-card class="mx-auto my-6 pa-3" :max-width="maxWidth">
      <v-form v-model="valid" ref="form">
        <v-row justify="center">
          <v-img
            :src="require('~/assets/imgs/priceAtMarketCap.png')"
        ...
        <v-row justify="center">
          <v-card-title
            class="
              ...
              mb-8
              font-weight-bold
              text-xl-h4 text-lg-h4 text-md-h4 text-sm-h4
              h5
            "
          >
            Price at Market Cap
        ...
        <v-row class="mx-8" justify="center">
          <p
            class="
              pa-0
              mb-6
              text-xl-h5 text-lg-h5 text-md-h5 text-sm-h5
              subtitle-1
            "
          >
            with market cap of
        ...
        <v-row>
          <v-btn
            ...
            name="calculate"
          >
            <v-icon left light> mdi-calculator </v-icon>
            Calculate
          </v-btn>
        </v-row>
      </v-form>
      ...
      <v-row
        ...
        justify="center"
      >
        <v-progress-circular class="text-center" indeterminate color="primary">
        </v-progress-circular>
      </v-row>
    </v-card>
      ...
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: this.priceAtMarketCapMetaInfo.description,
        },
        {
          hid: 'og:title',
          name: 'og:title',
          content: this.priceAtMarketCapMetaInfo.title,
        },
        {
          hid: 'og:image',
          property: 'og:image',
          content: require('~/assets/imgs/priceAtMarketCap.png'),
        },
        {
          hid: 'og:image:alt',
          property: 'og:image:alt',
          content: 'Price at Marketcap Image'
        },
        {
          hid: 'og:description',
  ...
  data() {
    return {
      ...
      priceAtMarketCapMetaInfo: {
        ...
        image: require("~/assets/imgs/priceAtMarketCap.png"),
        url: `${this.$router.currentRoute.path}`,
      },
      baseCoinRules: [(v) => !!v || "Coin is Required"],
      marketCapMultipleValueRules: [(v) => !!v || "Value is Required"],
      marketCapMultipleCoinRules: [(v) => !!v || "Coin is Required"],
    };
  },
  created() {
  ...
  methods: {
    ...
    async calcPriceAtMarketCap_inputCheck() {
      ...
      if (this.baseCoin == this.marketCapMultipleCoin) {
        this.priceAtMarketCapErrorMessage =
          "Coins are the same. Please select different coins!";
    ...
    async calcPriceAtMarketCap() {
      ...
      let fullURL_multipleCoin =
        baseURL + this.marketCapMultipleCoin + apiParams;
      ...
      try {
        ...
        this.isBaseStableCoin = await this.isStableCoin(
          this.baseCoinData.categories
        );
        ...
        this.multipleCoinMarketCapRank = this.multipleCoinData.market_cap_rank;
        this.multipleCoinPrice =
          this.multipleCoinData.market_data.current_price.usd;
        // console.log(this.baseCoinData)
        // this.multipleCoinName = this.multipleCoinData.name
        this.multipleCoinMarketCap =
          this.multipleCoinData.market_data.market_cap.usd;
        // this.multipleCoinPrice = this.multipleCoinData.market_data.current_price.usd

        this.marketCapMultiple =
          this.multipleCoinMarketCap / this.baseCoinMarketCap;

        if (!this.isBaseStableCoin) {
          this.priceAtNewMarketCap = this.formatPrice(
            this.baseCoinPrice * this.marketCapMultiple
          );
        } else {
