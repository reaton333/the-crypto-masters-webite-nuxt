<template>
  <v-card v-if="response.coins" class="mx-auto mt-8 pa-3" max-width="1000">
    <v-form v-model="valid" ref="form">
      <v-container>
        <v-row justify="center">
          <v-col class="shrink">
            <v-img
              :src="require('~/assets/imgs/professor_hindsight.png')"
    ...
    <div class="text-center" v-if="loadingCalculation">
      <v-progress-circular indeterminate color="primary"> </v-progress-circular>
    </div>
    <div v-if="newAmount !== ''">
      <v-card-title>
        <v-row align="start">
          <div class="text-caption grey--text text-uppercase">Total Profit</div>
          <div>
            <span
              class="font-weight-bold"
              :class="
                newAmount - amountInvested >= 0
                  ? 'success--text'
                  : 'error--text'
              "
            >
      ...
      <v-row>
        <v-col cols="10" md="4">
          <v-card-subtitle
            ...
            "
          >
            <p class="mb-0 font-weight-bold">Starting Price</p>
        ...
        <v-col cols="10" md="4">
          <v-card-subtitle
            ...
            "
          >
            <p class="mb-0 font-weight-bold">Ending Price</p>
        ...
        <v-col cols="10" md="4">
          <v-card-subtitle
            ...
            "
          >
            <p class="mb-0 font-weight-bold">New Amount</p>
...
export default {
      ...
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: `See what could have happened if you invested`,
        },
        {
          hid: 'og:title',
          name: 'og:title',
          content: `Professor Hindsight - Past Price Predictions`,
        },
        {
          hid: 'og:image',
          property: 'og:image',
          content: require('~/assets/imgs/professor_hindsight.png'),
        },
        {
          hid: 'og:image:alt',
          property: 'og:image:alt',
          content: 'Professor Hindsight Image'
        },
        {
          hid: 'og:description',
  ...
  data() {
    return {
      ...
      //   image: require("~/assets/imgs/professor_hindsight.png"),
      ...
      investedRules: [(v) => !!v || "Amount Invested is Required"],
      coinIdRules: [(v) => !!v || "Coin Selection is Required"],
    };
  },
  ...
  created() {
    ...
    this.whatIfStartDate = new Date()
    this.whatIfStartDate.setFullYear(this.whatIfStartDate.getFullYear() - 1);
    // Now format the date
    this.whatIfStartDate = this.whatIfStartDate.toISOString().substr(0, 10);
  },
  mounted() {},
  watch: {
    ...
    startDateMenu(val) {
      val &&
        setTimeout(() => (this.startDateActivePicker = "YEAR"));
    },
    endDateMenu(val) {
  ...
  methods: {
    ...
    async getWhatIfData() {
      ...
      try {
        ...
        this.sparklineValue = pricesArr.map(function (value, index) {
          return value[1];
        });
        ...
        this.totalProfit = this.formatPrice(
          this.newAmount - this.amountInvested
        );
